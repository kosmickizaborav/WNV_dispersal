...
embc_fldr <- "8_migration_sum"

# OUTPUT
out_dirs <- file.path(study_dir, gsub(" ", "_", target_sp), embc_fldr)
...
files_out <- gsub("5_flag_static", embc_fldr, files)
...
if(nf > 0){
  ...
  lapply(seq_along(files), function(n){
    ...
    # output directory
    fout <- gsub("5_flag_static", embc_fldr, fin)
    ...
    track[, weekly_bursts := cumsum(time_gap > window_size)]
    ...
    #yrange <- abs(max(track$y_) - min(track$y_))
...
if(nsp > 0){
  ...
  lapply(seq_along(sp_to_do), function(n){
    ...
    sp_dir <-  file.path(study_dir, gsub(" ", "_", sp), embc_fldr)
...
lapply(seq_along(target_sp), function(n){
  ...
  mig_dt |>
    ...
    geom_bar(
      aes(
        x = as.factor(embc_clst_lab), 
        y = after_stat(count), 
        fill = as.factor(embc_clst_lab)), 
      alpha = 0.3
    ) +
    scale_fill_manual(
      values = c("HH" = "blue", "HL" = "orange", "LH" = "red", "LL" = "green"), 
  ...
  ggsave(
    filename = file.path(
      plots_dir, paste0("3_", gsub(" ", "_", sp), "_migration_histograms.png")
    ),
    width = 10, height = 6, dpi = 300, bg = "white"
  )
